...
<body>
	...
	<div class="containerlist"><br>
		<span style="color: #005200;"><h2>Books Listings</h2></span><br>
		<br>
        <div class="imglist">
		<?php 
            $query_books = "SELECT * FROM product WHERE tags = 'Books'";
            $result_books = mysqli_query($conn,$query_books);
            while (($row = mysqli_fetch_array($result_books))) {?>
            <div class='listing-container-list'>
            <form method="post" action="bookslistings.php?ids=<?php echo $ids; ?>&id=<?=$row['productID'] ?>">
