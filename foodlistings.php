...
<body>
	...
	<div class="containerlist"><br>
		<span style="color: #005200;"><h2>Food Listings</h2></span><br>
		<br>
        <div class="imglist">
		<?php 
            $query_food = "SELECT * FROM product WHERE tags = 'Food'";
            $result_food = mysqli_query($conn,$query_food);
            while (($row = mysqli_fetch_array($result_food))) {?>
            <div class='listing-container-list'>
            <form method="post" action="foodlistings.php?ids=<?php echo $ids; ?>&id=<?=$row['productID'] ?>">
